...
func (optr *Operator) isMachineConfigPoolConfigurationValid(pool *mcfgv1.MachineConfigPool, version, releaseVersion, newFormatOsURL, osURL string, co *configv1.ClusterOperator) error {
	...
	for _, mcName := range mcs {
		mc, err := optr.mcLister.Get(mcName)
		if err != nil {
			return err
		}
	...
	// rolled out
	renderedMC, err := optr.mcLister.Get(pool.Status.Configuration.Name)
	if err != nil {
		return err
	}
