...
func ConvertRawExtIgnitionToV3_3(inRawExtIgn *runtime.RawExtension) (runtime.RawExtension, error) {
	...
	outIgnV32, err := json.Marshal(ignCfgV33)
	if err != nil {
		return runtime.RawExtension{}, fmt.Errorf("failed to marshal converted config: %w", err)
	}
...
// ConvertV3ToV2Ignition is used ONLY in the machine-config-server to downtranslate the
// password section of the ignition so we can inject it into the encapsulated config
// so it's present during firstboot.
func ConvertV3ToV2Ignition(cfg ign3types.Config) (ign2types.Config, error) {
	return convertIgnition34to22(cfg)
}

// ConvertRawExtIgnitionToV2 ensures that the Ignition config in
// the RawExtension is spec v2.2, or translates to it.
func ConvertRawExtIgnitionToV22(inRawExtIgn *runtime.RawExtension) (runtime.RawExtension, error) {
