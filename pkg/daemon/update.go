...
func (dn *CoreOSDaemon) experimentalUpdateLayeredConfig() error {
	...
	if onDesired, needRebase := onDesiredImage(desiredImage, booted, staged); !onDesired {
		...
		// but most likely activeChecksum != staged.Checksum, so we need to apply live or reboot
	...
	if dn.nodeWriter != nil {
		dn.nodeWriter.Eventf(corev1.EventTypeNormal, "NodeDone", fmt.Sprintf("Setting node %s, currentConfig %s to Done", dn.node.Name, desiredImage))
	}
