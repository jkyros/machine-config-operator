...
func (dn *CoreOSDaemon) validateKernelArguments(currentConfig *mcfgv1.MachineConfig) error {
	...
	for _, karg := range unexpectedIgnition {
		if _, ok := foundArgs[karg]; ok {
			erroneously_present = append(erroneously_present, karg)
		}
	}
	if len(erroneously_present) > 0 {
		cmdlinebytes, err := os.ReadFile(CmdLineFile)
		if err != nil {
			klog.Warningf("Failed to read %s: %v", CmdLineFile, err)
		} else {
		...
		return fmt.Errorf("erroenously present kernel arguments: %v", erroneously_present)
	}
